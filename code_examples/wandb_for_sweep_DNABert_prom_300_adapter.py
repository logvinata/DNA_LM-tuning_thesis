...
phm_param = {'phm_layer' : False, 'phm_dim' : 4, 'factorized_phm_W' : True, 
             'shared_W_phm' : False, 'shared_phm_rule' : True, 
...
def training_script(parameters=parameters, small=True, log = True):
    ...
    if small:
        ...
        note = "prom_300 sweep"
    run = wandb.init(project="DNA_BERT_sweeps_prom_300", job_type="adapter_train_hyperoptimization", 
                     config=parameters, save_code=True, 
                     notes=note)
    parameters = wandb.config
    print("parameters: ", parameters)
    # log the data usage
    run.use_artifact('logvinata/DNA_BERT_prom_300/prom_300_splitted_6_mers:v0', type='6mers')
    run.log_code(".", include_fn=lambda path: "./DNA_BERT / wandb_for_sweep_DNABert_prom_300_adapter.py") 
