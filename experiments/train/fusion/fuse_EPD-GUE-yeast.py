...
def save_all_fusion(
    ...
    try:
        outputs = model(
            **test[0]["token_type_ids"], output_adapter_fusion_attentions=True
        )
        ...
        artifact = wandb.Artifact(
            f"adapters_fusion_attention_{model_name}",
        ...
        run.log_artifact(
            artifact, aliases=[f"{dataset_name}_{fusion_name}_attention_fusion"]
        )
    except Exception:
        pass
    finally:
