...
def training_script(trial=optuna.trial.Trial, small=True, log=True):
    ...
    parameters = {
        ...
        "learning_rate": trial.suggest_loguniform(
            name="learning_rate", low=2e-6, high=2e-3
        ),
        ...
        "reduction_factor": trial.suggest_categorical("reduction_factor", [2, 4, 16]),
        "non_linearity": trial.suggest_categorical(
            "non_linearity", ["swish", "gelu", "relu"]
        ),
        "original_ln_before": trial.suggest_categorical(
            "original_ln_after", choices=[False, True]
        ),
...
if __name__ == "__main__":
    ...
    wandb_kwargs = {
        "project": "DNA_BERT_sweeps_promoters_2000",
        "job_type": "adapter_train_hyperoptimization",
        "group": "DDP",
        "save_code": True,
    }
    ...
    study.optimize(
        lambda trial: training_script(trial, small=small, log=True),
        n_trials=n_trials,
        callbacks=[wandbc],
    )
