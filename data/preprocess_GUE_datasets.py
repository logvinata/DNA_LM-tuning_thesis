...
#         # the split is already there 
        ...
        train = load_dataset(
                        'csv', 
                        data_files=f"{split_path}train.csv", 
                        delimiter=","
                       )['train']
        
        val = load_dataset(
                        'csv', 
                        data_files=f"{split_path}dev.csv", 
                        delimiter=","
                       )['train']
        
        test = load_dataset(
                        'csv', 
                        data_files=f"{split_path}test.csv", 
                        delimiter=","
                       )['train']
        ...
        split_data = wandb.Artifact(f"{dataset_name}_splitted_data", type="splitted_dataset", 
        ...
        k_mer_train = train.map(lambda batch: {"kmer": [return_kmer_simple(seq) for seq in batch["sequence"]]}, 
                            remove_columns=["sequence"], batched=True, batch_size=len(train))
        k_mer_val = val.map(lambda batch: {"kmer": [return_kmer_simple(seq) for seq in batch["sequence"]]}, 
                            remove_columns=["sequence"], batched=True, batch_size=len(val))
        k_mer_test = test.map(lambda batch: {"kmer": [return_kmer_simple(seq) for seq in batch["sequence"]]}, 
                            remove_columns=["sequence"], batched=True, batch_size=len(test))
        ...
        with wandb.init(
            ...
            artifact = run.use_artifact(f'logvinata/{project_name}/{dataset_name}_splitted_data:latest', 
                                        type='splitted_dataset')
            artifact = wandb.Artifact(f'{dataset_name}_splitted_6_mers', type="6mers", 
