...
def load_adapter_model(parameters):
    ...
    model_config = AutoConfig.from_pretrained("zhihan1996/DNA_bert_6", 
...
def add_adapter(model, parameters):
    ...
    model.add_classification_head(parameters['task_name'], num_labels=parameters['num_labels'], overwrite_ok=True) #parameters['num_labels'])
...
def add_trained_adapter(model, adapter_name, target_task, with_head=False):
    ...
    model.load_adapter(f"./adapters/DNABERT_{adapter_name}_adapter/{adapter_name}", 
                       load_as=f"{adapter_name}", with_head=with_head)
...
def save_all_fusion(run, trainer, model, test, fusion_name, save_model=False, save_adapter=True, save_fusion = True, is_sweep=False):
...
#         artifact = wandb.Artifact(f'adapters_for_fusion_{model_name}', type="adapters_set", 
#                                      description=f"DNABERT_adapters_for_fusion_{fusion_name}")
#         artifact.add_dir(f"./adapters_for_fusion_/{model_name}")
#         # ./adapters_for_fusion_/DNABERT_fusion_GUE_all_tfs_on_human_3_tf_100
#         run.log_artifact(artifact, aliases=[f'{dataset_name}_{fusion_name}_adapters_set'])
        
    if save_fusion:
        model.save_adapter_fusion(f"./adapters_fusion/{model_name}", adapters_list)
        model.save_head(f"./adapters_fusion/{model_name}", dataset_name)
        artifact = wandb.Artifact(f'adapters_fusion_{model_name}', type="adapters_fusion", 
    ...
    try:    
        outputs = model(**test[0]['token_type_ids'], output_adapter_fusion_attentions=True)
        attention_scores = outputs.adapter_fusion_attentions

        with open(f"./adapters_fusion/{model_name}/attentions.txt", "w") as f:
            f.write(attention_scores)
        artifact = wandb.Artifact(f'adapters_fusion_attention_{model_name}', type="fusion_attention", 
                                  description=f"attention_scores_for_DANBERT_{fusion_name}_fusion_tuned_on_GUE_{dataset_name}")

        artifact.add(attention_scores)
        run.log_artifact(artifact, aliases=[f'{dataset_name}_{fusion_name}_attention_fusion'])
    except Exception:
        pass
    finally:
...
def training_script(small=True, log = True, task_parameters = {}):
    ...
    parameters = {
        ...
        'save_total_limit' : 3,
        'learning_rate' : 6e-6, 
        # 0.00016016789158802676,  # trial.suggest_loguniform(name='learning_rate', low=2e-6, high=2e-3),
...
#         parameters[k] = v
    ...
    if int(task_parameters['length']) > 300:
        parameters['batch_size'] = parameters['batch_size'] // 4
    ...
    run=wandb.init(project=project_name, job_type="fusion_training", name=f"{model_name}", 
    ...
    model = add_and_fuse(model, task_parameters['adapters_list'], task_parameters['dataset_name'])
...
#     mcc = eval.load("matthews_correlation")
    ...
    score = save_all_fusion(run, trainer, model, test, 
                     fusion_name = task_parameters['fusion_name'],
...
if __name__ == '__main__':
    ...
    os.environ['WANDB_LOG_MODEL'] = 'end'
    fusion_name = "GUE_yeast_EMP"
    adapters_list = fusion_tasks.GUE_yeast_EMP
