...
if __name__ == '__main__':
    set_seed(0)
    x = [f.name for f in os.scandir(f"../../datasets/promoters/EPD_data/EPD_data_csv") if f.is_file()]
    for f in x:
        organism, genome, task, length, _ = f.split("_")
        print(organism, task, genome, length)
        dataset_name = f"{organism}_{genome}_{task}_{length}"
        notebook_name = f"DNABERT_human_full_evaluate_on_{dataset_name}"  # just convenience
        ...
        dataset_art_name = f'logvinata/{task}_{length}_datasets/{dataset_name}_splitted_6_mers:latest'
        model_art_name = 'logvinata/model-registry/DNA_BERT_full_prom_300:v2'
        
        # OLD VERSION
        parameters = {'seed' : 0, 'model_name': 'zhihan1996/DNA_bert_6', 'task_name' : 'prom_300',
              'num_train_epochs' : 10, 'save_total_limit' : 7, 'weight_decay' : 0.02, 
        ...
        run=wandb.init(project=project_name, job_type="evaluation", name=f"{model_name}_on_{genome}", 
                       config=parameters, save_code=True, notes=notebook_name, 
                       group = organism, tags = [model_name, organism])
